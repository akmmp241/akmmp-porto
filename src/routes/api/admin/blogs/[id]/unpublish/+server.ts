...
import { BlogServiceError, unpublishPost } from '$lib/server/blog-service';
...
export async function POST(event: RequestEvent): Promise<Response> {
	...
	try {
		...
		await logAction(actorAuditUserId(auth.actor), 'blog.unpublish', `blog:${id}`, actorMetadata(auth.actor), event.getClientAddress());
		return json({ post });
	} catch (error) {
